...
namespace boost
{
  namespace any_detail
  {
    ...
    R match_any(any& x, meta::types<T1>, F&& f)
    {
    ...
    R match_any(any& x, meta::types<T1, T2, Ts...>, F&& f)
    {
      T1 * res = any_cast<T1>(&x);
      if (res) return f(*res);
      else return match_any<R>(x, meta::types<T2, Ts...>(), std::forward<F>(f));
    }

    template <class R, class T, class ...Ts, class F1, class F2, class ...Fs>
    R match_any(any& x, meta::types<T, Ts...>, F1&& f1, F2&& f2, Fs&&... fs)
    {
      return match_any<R>(x, meta::types<T, Ts...>(),
          functional::overload(std::forward<F1>(f1), std::forward<F2>(f2), std::forward<Fs>(fs)...)
      );
    }
    template <class R, class Types, class ...Fs>
    R match_any(functional::selector<any, Types> && x, Fs&&... fs)
    {
