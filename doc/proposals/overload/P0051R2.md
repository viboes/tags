...
* Ensure that forward cv and ref qualifiers are forwarded correctly.
* Check the wording with someone from the LGW before sending a new revision to LWG.
...
*Remarks*: This function as well as the overloaded `operator()` for each `fcts` on the resulting type shall be a `constexpr` functions. The overloaded `operator()` for each `fcts` on the resulting type shall be `noexcept(noexcept(fcts()))`.
```c++
    template <class ... Fs>
    constexpr 'see below' overload(Fs &&... fcts) noexcept('see below');
